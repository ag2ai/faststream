...
class MySQLSqlaClient(SqlaClient):
    async def fetch(
        self,
        queues: list[str],
        *,
...
def create_sqla_client(engine: AsyncEngine) -> SqlaClient:
    """Return a dialect-aware client implementation."""
    dialect = engine.dialect.name.lower()
    if dialect.startswith("mysql"):
        return MySQLSqlaClient(engine)
    return SqlaClient(engine)
