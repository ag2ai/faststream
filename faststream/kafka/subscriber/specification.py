...
class KafkaSubscriberSpecification(
    ...
    def topics(self) -> list[str]:
        ...
        topics.update(f"{self.outer_config.prefix}{t}" for t in self.config.topics)

        topics.update(
            f"{self.outer_config.prefix}{p.topic}" for p in self.config.partitions
        )

        if self.config.pattern:
            topics.add(f"{self.outer_config.prefix}{self.config.pattern}")
