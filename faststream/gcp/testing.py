...
__all__ = ("TestGCPBroker",)


class TestGCPBroker(TestBroker[GCPBroker]):
    """A class to test GCP Pub/Sub brokers."""

    @contextmanager
    def _patch_producer(self, broker: GCPBroker) -> Iterator[None]:
        fake_producer = FakeGCPProducer(broker)
    ...
    @staticmethod
    async def _fake_connect(broker: GCPBroker, *args: Any, **kwargs: Any) -> None:
        """Fake connection method."""


class FakeGCPProducer:
    """A fake GCP Pub/Sub producer for testing purposes."""

    def __init__(self, broker: GCPBroker) -> None:
        self.broker = broker
    ...
    async def publish_batch(
        ...
        if isinstance(cmd, list):
            # List of GCPPublishCommand objects
            for command in cmd:
            ...
            for msg in cmd.messages:
                # Create proper GCPPublishCommand
    ...
    async def _execute_handler(
        ...
        # The parser will wrap it in GCPMessage
