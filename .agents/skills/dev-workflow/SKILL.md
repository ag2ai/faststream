...
- `just up` / `just stop` / `just down` — start / stop / remove all broker containers.
- Per broker: `just kafka-up`, `rabbit-up`, `nats-up`, `redis-up`, `redis-cluster-up` (+ matching `-stop` and `-logs` recipes).
...
| `just test-<broker> [path]` | broker-marked tests excluding `connected` (kafka, confluent, rabbit, redis, redis-cluster, nats) |
...
1. `just linter` — ruff format + ruff check --fix + codespell (alias: `just lint`).
