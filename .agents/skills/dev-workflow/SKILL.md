...
3. `just static-analysis` — mypy + pyright + bandit + semgrep + import-linter (`just import-linter` alone for the import contracts); `just zizmor` (security) and `just actionlint` (syntax, expressions, shellcheck over `run:` scripts) separately for GitHub Actions workflows.
