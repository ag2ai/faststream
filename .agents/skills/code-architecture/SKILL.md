...
- `faststream/_internal/` holds shared machinery: `broker/` (abstract `BrokerUsecase`, registrator, router), `endpoint/`, `di/` (fast-depends integration), `context/`, `configs/`, `testing/`, `cli/`, `fastapi/`, `utils/`.
...
├── subscriber/        # subscriber endpoint + usecases/
...
- Generics are used for broker abstractions: `BrokerUsecase[ConnectionType, MsgType]`, `BaseMiddleware[PublishCommandType, AnyMsg]`.
