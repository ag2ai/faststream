...
- `just test-kafka` / `test-rabbit` / `test-nats` / `test-redis` / `test-redis-cluster` / `test-confluent` — per-broker fast subset; `-all` variants include connected tests (need `just up` or `just <broker>-up` first).
- `just test-all` — everything including connected/slow.
- Direct: `uv run pytest tests/... -m "not slow and not connected"`.
...
- Talks to a real broker over the network → add `@pytest.mark.connected()` (excluded from the default run).
...
- Global fixtures (`tests/conftest.py`): `queue` (unique uuid string), `event` (`asyncio.Event`), `mock` / `async_mock` (session-scoped, auto-reset between tests), `context`, `runner` (CLI).
