...
Beside these fields, the handlers and publishers of a broker take that broker's own, under the names its `publish()` uses for them. A Kafka handler or publisher takes `key` and `partition`, compared exactly as the client delivered them. A RabbitMQ handler or publisher takes `exchange`, `routing_key`, `message_id` and `priority`, read off the message as it was delivered. A NATS handler or publisher takes `subject`, the whole address the message arrived on, where `path` holds only what the template captured:
...
=== "NATS"
    ```python linenums="1" hl_lines="6 16 20-24"
    {!> docs_src/getting_started/subscription/nats/advanced_testing.py [ln:3,5-8,92-110] !}
    ```
