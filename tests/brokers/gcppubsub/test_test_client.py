...
@pytest.mark.asyncio()
@require_gcppubsub
class TestTestclient(GCPPubSubMemoryTestcaseConfig, BrokerTestclientTestcase):
    ...
    async def test_fake_producer_publish(self, topic: str) -> None:
        ...
        async with self.patch_broker(broker) as br:
            ...
            fake_producer = FakeGCPPubSubProducer(br)
    ...
    async def test_fake_producer_batch_publish(self, topic: str) -> None:
        ...
        async with self.patch_broker(broker) as br:
            ...
            commands = [
                GCPPubSubPublishCommand(message=f"msg{i}", topic=topic) for i in range(3)
            ]
