...
class MQTTRequestsTestcase(RequestsTestcase):
    ...
    async def test_broker_publisher_request_respect_middleware(self, queue: str) -> None:
        self._skip_if_v311()
        await super().test_broker_publisher_request_respect_middleware(queue)

    async def test_router_publisher_request_respect_middleware(self, queue: str) -> None:
        self._skip_if_v311()
