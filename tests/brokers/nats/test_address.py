...
class TestNatsAddressTemplate(NatsMemoryTestcaseConfig, AddressCheckTestcase):
    ...
    def test_an_early_read_does_not_pin_an_outer_router_prefix(self) -> None:
        """The read NATS refuses, in place of the one it used to defend against.

        A Router included into another Router composes a longer prefix than its
        endpoints saw when they were declared. Asked in between, a NATS
        Subscriber says the read came too early instead of answering with a
        subject built from a composition that is not final.
        """
