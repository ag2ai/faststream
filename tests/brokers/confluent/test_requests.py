...
@pytest.mark.confluent()
class TestRequestSkipNoneConnected(ConfluentTestcaseConfig):
    @pytest.mark.asyncio()
    async def test_request_skips_none(self, queue: str) -> None:
        """`skip_none` is checked in `_basic_request` before the producer.

        Confluent doesn't support RPC: `AsyncConfluentFastProducer.request`
        raises `FeatureNotSupportedException`. This connected test passes only
        because the guard short-circuits inside the shared publisher pipeline
        (`PublisherUsecase._basic_request`), so the producer (and its
        exception) is never reached. Moving the guard into the producer would
        break this test — which is exactly the invariant it pins.
        """
